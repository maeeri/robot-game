...
class Game:
    ...
    def read_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if (self.game_over or self.victory) and (
    ...
    def collect_coin(self):
        coin_sound = pygame.mixer.Sound(os.path.join(dirname, "sound/coin.mp3"))
        victory_sound = pygame.mixer.Sound(os.path.join(dirname, "sound/victory.wav"))
        for c in self.coins:
            condition_x = (
                abs(c.center_x - self.robot.center_x)
                <= (c.width + self.robot.width) / 2
            )
            condition_y = (
                abs(c.center_y - self.robot.center_y)
                <= (c.height + self.robot.height) / 2
            )
            if condition_x and condition_y:
                c.random_coin_placement()
    ...
    def encounter_monster(self):
        monster_sound = pygame.mixer.Sound(os.path.join(dirname, "sound/life_lost.wav"))
        game_over_sound = pygame.mixer.Sound(
            os.path.join(dirname, "sound/game_over.wav")
        )
        for m in self.monsters:
            condition_x = (
                abs(m.center_x - self.robot.center_x)
                <= (m.width + self.robot.width) / 2
            )
            condition_y = (
                abs(m.center_y - self.robot.center_y)
                <= (m.height + self.robot.height) / 2
            )
            if condition_x and condition_y:
    ...
    def control_monsters(self):
        ...
        if (
            len(self.monsters) == 1
            and self.points >= 4
            or len(self.monsters) == 4
            and self.points >= 12
        ):
            self.monsters.append(
                Monster(self.screen_width, self.screen_height, False, False, True)
            )
        if (
            len(self.monsters) == 2
            and self.points >= 7
            or len(self.monsters) == 5
            and self.points >= 14
        ):
            self.monsters.append(
                Monster(self.screen_width, self.screen_height, False, True, True)
            )
        if len(self.monsters) == 6 and self.points >= 17:
            self.monsters.append(
                Monster(self.screen_width, self.screen_height, True, False, False)
            )
    ...
    def game_over_screen(self):
        ...
        line = body_font.render(
            "Return to start screen by pressing 'Enter'", True, ((255, 255, 255))
        )
        self.screen.blit(
            title,
            (
                (self.screen_width - title.get_width()) / 2,
                (self.screen_height - title.get_height()) / 2 - 20,
            ),
        )
        self.screen.blit(
            line,
            (
                (self.screen_width - line.get_width()) / 2,
                (self.screen_height - title.get_height()) / 2 + 40,
            ),
        )
    ...
    def start_screen(self):
        ...
        for i in range(len(contents)):
            ...
            self.screen.blit(
                line, ((self.screen_width - line.get_width()) / 2, 230 + i * 25)
            )

        robo = pygame.image.load(os.path.join(dirname, "img/robo.png"))
    ...
    def points_display(self):
        pygame.draw.rect(
            self.screen, (0, 0, 0), pygame.Rect(self.screen_width - 140, 10, 130, 70)
        )
